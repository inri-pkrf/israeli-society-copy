...
const Menu = () => {
  ...
  const [pressedButtons, setPressedButtons] = useState(() =>
    JSON.parse(sessionStorage.getItem('pressedButtons')) || { dos: false, arab: false, old: false }
  );
  ...
  const toggleDropdown = (path) => {
    setOpenDropdown((prev) => ({ ...prev, [path]: !prev[path] }));
  };
  ...
  const handleMenuClick = (item) => {
    ...
    else if (item.prompt === 'החברה הערבית')      handlePress('arab', item.prompt);
    else if (item.prompt === 'מוגבלויות והגיל השלישי') handlePress('old', item.prompt);
    else navigate(path);
  };
  ...
  const parts = [
    ...
    { name: 'חלק ראשון', path: '/part-one', locked: false,
      subtopics: [{ name: 'רבדי החברה הישראלית', path: '/part-one' }] },
    { name: 'חלק שני', path: '/part-two', locked: !visitedPages.includes('/part-one'),
      subtopics: [
    ...
    { name: 'חלק שלישי', path: '/part-three', locked: !allPromptsViewed,
      subtopics: [
        { name: 'החברה הישראלית לערבית', path: '/game-intro' },
        { name: 'המשחק', path: '/game-explaine' },
        { name: '10 נקודות שחשוב לזכור', path: '/summary-points' },
      ] },
    { name: 'בוחן סיום', path: '/final-screen', locked: !gameStepsViewed, subtopics: [] },
  ];
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  {part.subtopics.length > 0 && !isDesktop && (
                    <img
                      ...
                      onClick={(e) => { e.stopPropagation(); toggleDropdown(part.path); }}
                    />
                  )}
                </div>
                  ...
                  <ul className="submenu-list-home">
                    {part.subtopics.map((subtopic, subIndex) => (
                      <li
                        ...
                        onClick={(e) => { e.stopPropagation(); handleMenuClick(subtopic); }}
                      >
                        {subtopic.name}
                      </li>
                    ))}
